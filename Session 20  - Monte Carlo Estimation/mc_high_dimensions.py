...
def main():
    ...
    for dim in np.arange(1, dimensions):
        ...
        v = halton(np.arange(dots), sympy.prime(dim)) * 2 - 1
        d += v**2
        est[dim] = np.count_nonzero(d <= 1.0) / dots * 2**dim

    act_x = np.linspace(0, dimensions - 1, 10_000)
    act_y = np.power(np.pi, act_x / 2) / gamma(act_x / 2 + 1)
    m = find_peaks(act_y)[0][0]
    print(f"Maximum Dimension = {act_x[m]:.6f}")
    print(f"Maximum Content = {act_y[m]:.6f}")
    ...
    plt.title("Content of n-Dimensional Hyperspheres")
    plt.xlabel("Dimension")
    plt.ylabel("Content")
    plt.grid()
    plt.legend()
    plt.gcf().gca().xaxis.set_major_locator(MultipleLocator(1))
    plt.gcf().gca().xaxis.set_minor_locator(MultipleLocator(0.5))
    plt.gcf().gca().yaxis.set_minor_locator(AutoMinorLocator())
    plt.show()
